...
export async function sendApprovalEmail(postId: number, title: string, summary: string, content: string): Promise<void> {
  if (!resend || !APPROVAL_EMAIL) {
    console.warn("Email gönderimi atlandı: RESEND_API_KEY veya APPROVAL_EMAIL eksik.");
    return;
  }
...
router.get("/approve/:id", async (req: Request, res: Response): Promise<void> => {
  const id = Number(req.params.id);
  const action = req.query.action as string;

  if (!id || !action) {
    res.status(400).send("Gecersiz istek.");
    return;
  }
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : "Bilinmeyen hata";
    res.status(500).send("Hata: " + message);
  }
});
