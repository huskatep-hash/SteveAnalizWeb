...
async function seed() {
  ...
  // Haberler - hap_numbers OLMADAN, sadece temel alanlar
  await pool.query(`INSERT INTO news_posts (title, slug, summary, content, category, tags, status, read_time, hap_headline, hap_context, hap_impact, hap_quote) VALUES 
  ...
  // Blog yazilari
  await pool.query(`INSERT INTO blog_posts (title, slug, summary, content, tags, type, status, author) VALUES 
  ...
  console.log('✓ Blog yazilari eklendi.');

  // Egitim
  await pool.query(`INSERT INTO education_posts (title, slug, summary, content, type, "order") VALUES 
  ...
  console.log('✓ Egitim icerikleri eklendi.');

  // Yazarlar
  await pool.query(`INSERT INTO writers (name, email, bio, status) VALUES 
  ...
  console.log('\n🎉 TUM SEED ISLEMLERI BASARIYLA TAMAMLANDI!');
  pool.end();
}
seed();
